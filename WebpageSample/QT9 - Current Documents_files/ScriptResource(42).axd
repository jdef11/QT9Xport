...
if(!at.is(".k-window-content")||!aF[0]){at.addClass("k-window-content");
...
az.wrapper.find(af).addClass("k-flat");
...
this._removeOverlay();
ay.wrapper.find(af).removeClass("k-flat");
...
ar=aq.find("iframe:not(.k-content-frame)").map(function(){var aw=this.getAttribute("src");
this.src="";
return aw;
});
av.toggleClass("k-rtl",at).append(aq).find("iframe:not(.k-content-frame)").each(function(aw){this.src=ar[aw];
});
...
ae={wrapper:ad("<div class='k-widget k-window'></div>"),action:ad("<a role='button' href='\\#' class='k-button k-flat k-button-icon k-window-action' aria-label='#= name #'><span class='k-icon k-i-#= name.toLowerCase() #'></span></a>"),titlebar:ad("<div class='k-window-titlebar'><span class='k-window-title'>#= title #</span><div class='k-window-actions'></div></div>"),overlay:"<div class='k-overlay'></div>",contentFrame:ad("<iframe frameborder='0' title='#= title #' class='"+w+"' src='#= content.url #'>This page requires frames in order to show content</iframe>"),resizeHandle:ad("<div class='k-resize-handle k-resize-#= data #'></div>")};
